...
class FakeSlice:
    def __init__(self, T, theta, n=23):
        self.T = T
        self.dte = T * 365.25
        self.forward = 500.0
        self.spot = 500.0
        self.forward_fit = SimpleNamespace(discount=0.995)
        self.k = np.linspace(-0.22, 0.18, n)
...
def test_fengler_calendar_grid_is_a_real_cap_and_reduced_solver_works():
    events = []
    fit = fit_fengler_surface(
        _slices(),
        smoothing_lambda=1e-6,
        calendar_grid_size=17,
        solver="slsqp",
        progress=events.append,
    )
    starts = [x for x in events if x["event"] == "start_slice"]
    assert starts[0]["calendar_points"] == 0
    assert all(x["calendar_points"] == 17 for x in starts[1:])
    assert fit.solver == "slsqp-reduced"
    assert fit.elapsed_seconds >= 0


def test_prepare_fengler_slices_limits_maturities_and_strikes():
    dense = []
    for d in range(20, 42, 2):
        T = d / 365.25
        theta = 0.04 * T + 0.001 * np.sqrt(T)
        dense.append(FakeSlice(T, theta, n=121))
    fast = prepare_fengler_slices(
        dense,
        target_days=30,
        max_maturities=5,
    ...
    assert len(fast) == 5
    assert all(s.n <= 61 for s in fast)  # ATM/endpoints can add at most one.
    assert min(s.dte for s in fast) < 30 < max(s.dte for s in fast)
